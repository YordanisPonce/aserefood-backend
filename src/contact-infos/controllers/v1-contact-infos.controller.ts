...
import UserUpdateInDto from '../../users/dto/in/user.update.in.dto';
...
export default class V1ContactInfosController{
  ...
  @Get('')
  @Roles(Role.Customer)
  @ApiOkResponse({ description: 'Ok', type: PaginatedOutDto<ContactInfoOutDto> })
  @ApiOperation({ summary: 'Get Contact Infos of the current customer with Filtering, Ordering and Pagination. Only for Customers' })
  async get(@Query() dto: ContactInfoSearchInDto, @Request() req): Promise<PaginatedOutDto<ContactInfoOutDto>> {
  ...
  @Roles(Role.Admin)
  @ApiOkResponse({ description: 'Ok', type: PaginatedOutDto<ContactInfoOutDto> })
  @ApiOperation({ summary: 'Get Contact Infos of the given customer with Filtering, Ordering and Pagination. Only for Admins' })
  async getBackOffice(@Query() dto: ContactInfoSearchInDto, @Param('id', ParseIntPipe) id: number): Promise<PaginatedOutDto<ContactInfoOutDto>> {
  ...
  @ApiOperation({summary: 'Get all Contact Infos of current customer. Only for Customers'})
  ...
  @Roles(Role.Admin)
  @ApiOkResponse({description: "Ok", type: [ContactInfoOutDto]})
  @ApiOperation({summary: 'Get all Contact Infos of given customer. Only for Admins'})
  ...
  @ApiBadRequestResponse({description: "Bad Request"})
  @ApiConflictResponse({description: 'Conflict (Other contact info with Name)'})
  @ApiOperation({summary: 'Create a new Contact Info if does not exist for current user. Only for Customers'})
  ...
  @ApiBadRequestResponse({description: "Bad Request"})
  @ApiConflictResponse({description: "Conflict (Other contact info with Name)"})
  @ApiOperation({summary: 'Update a Contact Info by its id. Only for Customers'})
  async put(
  ...
  @Roles(Role.Customer)
  @ApiOkResponse({description: "Ok"})
  @ApiConflictResponse({description: "Conflict (Current user's contact Info cannot be deleted.  Contact Info with Pending Orders)"})
  @ApiNotFoundResponse({description: "Not Found"})
  @ApiBadRequestResponse({description: "Bad Request"})
  @ApiOperation({summary: 'Delete PERMANENTLY a Contact Info by its id. Only for Customers'})
